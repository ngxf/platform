discovery.setPrepare(data => {
  discovery.addQueryHelpers({
    ...
    components: current => discovery.query("$.classes().[decorators().name().[$=\"Component\"]]", current),
    directives: current => discovery.query("$.classes().[decorators().name().[$=\"Directive\"]]", current),
    pipes: current => discovery.query("$.classes().[decorators().name().[$=\"Pipe\"]]", current)
  });
});
